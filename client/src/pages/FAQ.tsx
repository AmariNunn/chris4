...
const INTAKE_URL = "/intake-form";
const ALL_EXPERIENCES_URL = "https://cal.com/dr.-nicole-cox-jordan?redirect=false";

const faqs: { question: string; answer: string; extraContent?: "get-started" }[] = [
  {
  ...
  {
    question: "Is a Membership Required?",
    answer:
      "No, but if you decide against a membership you are not eligible for complimentary consultations or membership perks.",
  },
  {
    question: "Do you accept insurance?",
    answer:
      "If you opt for Chiropractic Care only, the answer is yes — we currently accept some insurances (AETNA, BCBS, United, UMR, Anthem). If your deductible for the year has been met, your insurance will pay a portion towards your chiropractic care. You may have a co-pay and/or a balance after each visit. If your deductible has not been fully met, your insurance will not pay anything towards your care and you are considered a CASH patient. Specific insurance policy details are verified and discussed with you before treatment is rendered.",
  },
];
      ...
      <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
        <Link href={INTAKE_URL}>
          <Button
            ...
            data-testid="button-faq-new-patients"
          >
            New Patients
          </Button>
        </Link>
        <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer">
          <Button
            size="sm"
            className="rounded-full px-6 bg-accent text-white hover:bg-accent/90"
            data-testid="button-faq-existing-patients"
          >
            Existing Patients
          </Button>
        </a>
        <a href={ALL_EXPERIENCES_URL} target="_blank" rel="noopener noreferrer">
          <Button
            size="sm"
            variant="outline"
            className="rounded-full px-6 border-2 border-primary/20 text-primary"
            data-testid="button-faq-all-experiences"
          >
            All Experiences
          </Button>
        </a>
...
function FAQItem({ question, answer, extraContent, index }: { question: string; answer: string; extraContent?: "get-started"; index: number }) {
        ...
        {open && (
          ...
          >
            <div className="px-7 pb-6 border-t border-border/40">
              ...
              {extraContent === "get-started" && <GetStartedContent />}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <section className="py-20 md:py-28 bg-secondary/20">
          <div className="container mx-auto px-4 md:px-6 max-w-3xl">
            <div className="space-y-4">
              {faqs.map((faq, i) => (
                <FAQItem key={i} question={faq.question} answer={faq.answer} extraContent={faq.extraContent} index={i} />
              ))}
            </div>
          </div>
        </section>
